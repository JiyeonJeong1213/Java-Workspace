...
public class A_UncheckedException {
	 ...
	 * - ArrayIndexOutOfBoundException : 배열에 부적절한 인덱스로 접근할 때 발생하는 예외
	 * - NegativeArraySizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외
	 * - ClassCastException : 허용할 수 없는 형변환이 진행될 경우 발생하는 예외
	 * - NullPointerException : 레퍼런스가 아직 null인데도 불구하고 접근하려고 할 때 발생하는 예외
	 * - ArithmeticException : 나누기 연산시 0으로 나눌 때 발생하는 예외
	 * - ........
	 * 
	 * => 이러한 RuntimException과 관련한 예외는 충분히 예상 가능한 상황이기 때문에
...
//		System.out.println("프로그램 종료");
		...
		// 해결방법 2. 예외처리 구문으로 해결(예외가 발생할 경우를 대비해서 실행할 내용을 정의해두는 것)
		/*
		 * try ~ catch문
		 * [표현법]
		 * try{
		...
		try {
			...
			//System.out.println("0으로 나눌 수 없습니다.");
			e.printStackTrace(); // 현재 발생한 오류를 추적할 수 있는 로그정보 제공
		}
		System.out.println("프로그램 종료");
	}
	...
	public void method3() {
		// ArrayIndexOutOfBoundsException : 배열에 부적절한 인덱스로 접근시 발생하는 예외
		// NegativeArraySizeException : 배열의 크기를 음수로 지정할 경우 발생하는 예외
		
		System.out.print("배열의 크기 : ");
		int size = Integer.parseInt(sc.nextLine());
		
		// 방법1. if문으로 size값이 양수값일 때만 생성하도록 제어
//		if(size>0) {
//			int[] arr = new int[size];
//		}
//		
		// 방법2. 항상 양의 정수값이 들어갈 수 있도록 abs메소드 사용
//		int[] arr = new int[Math.abs(size)];
...
//		} // 다중 catch블럭이 줄었다. 정확히 어떤 에러인지 알 수 없음
		...
		try {
			...
			System.out.println("예외가 발생하긴 했네요. 근데 배열의 크기를 음수로 입력했거나 부적절한 인덱스로 접근한 것 같습니다.");
		} // 다중 catch문에서는 블럭의 순서가 중요 => 범위가 작은 자식타입의 예외 클래스를 먼저 기술해야 함
	}
	
	/*
	 * RuntimeException 관련된 예외는
	 * - 조건문으로 해결 가능 => 예외 자체가 발생 안되게끔 개발자가 소스코드로 핸들링 가능
	 * - 예외처리 구문으로도 해결 가능 => 예외가 발생햇을 때를 대비해서 그 때 실행할 내용을 정의해두는 것
	 * 
	 ...
	 * 
	 * UncheckedException -> 필수로 체크하지 않아도 되는 예외라는 뜻
	 */
}
