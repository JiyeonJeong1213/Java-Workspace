...
public class ListRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 프레임워크?
		 * 효율적인 기능들이 정의되어 있는 틀(프레임)
		 * 
		 * 정리해보면
		 * 데이터들이 새로이 추가되거나, 삭제가 되거나, 수정이 되는 기능(알고리즘)들이 이미 정의되어 있느 틀이 있다.
		 * == 컬렉션
		 * => 다량의 데이터들을 관리하고자 할 때 배열 가지고 충분히 사용했었음. 단, 배열의 단점들을 보완한 것이 컬렉션.
		 * 
		 * 배열과 컬렉션의 차이점
		 * - 배열의 단점
		 * 1. 배열을 쓰고자 할 때 먼저 크기글 지정해야 함
		 * 	 => 한번 지정된 크기는 변경이 불가
		 * 		새로운 값을 추가하고자 할 때 크기가 오버될 경우 새로운 크기의 배열을 만들고 기존의 내용들을 복사해주는
		 * 		코드를 직접 구현해야 함
		 * 2. 배열 중간 위치에 새로운 데이터를 추가하거나 삭제해야 하는 경우 기존의 값들을 땡겨주는 코드를 직접 짜야 함
		 * 3. 한 타입의 데이터들만 저장 가능
		 * 
		 * - 컬렉션의 장점
		 * 1. 크기에 제약이 없다
		 * 	 => 크기지정을 해줄 필요도 없고 만일 크기지정을 해놔도 알아서 크기가 늘어나면서 새로운 데이터를 추가해주는 
		 * 		코드가 이미 정의되어 있음
		 ...
		 * 컬렉션의 3가지 분류
		 * 
		 * List계열 => 담고자 하는 값(value)만 저장/ 값 저장 시 순서유지 됨(index개념이 있음)/ 중복값 허용
		 * 			ex) ArrayList, Vector, LinkedList => ArrayList가 제일 많이 쓰임(90%이상)
		 * Set계열 => 담고자 하는 값(value)만 저장/ 값 저장 시 순서유지X(index개념 없음)/ 중복값 허용X
		 * 			ex) HashSet, TreeSet
		 * Map계열 => 키(key) + 담고자하는 값(value)세트로 저장/ 값 저장 시 순서유지X(index개념 없음)/ key 중복 허용X
		 * 			ex) HashMap, Properties
		 */
		
		ArrayList<Music> list  = new ArrayList<>(3); // 크기 지정 안 하면 내부적으로 크기 10짜리인 배열이 생성됨
		...
		//list.add("흐흐흐"); // 제네릭이 Music타입으로 제시되어 있어 추가 불가
		/*
		 * 순서가 유지되면서 값 추가됨
		 * 각 index에 담겨 있음
		 * 크기에 제약이 없음
		...
		// 3. set(int index, E e) : 리스트에 원하는 위치에 인덱스 값을 제시하면 전달되는 e 값으로 변경해주는 메소드
		list.set(0, new Music("피 땀 눈물", "BTS"));
		
		System.out.println(list);
		
		// 4. remove(int index) : 리스트에서 해당 인덕스 위치에 있는 값을 삭제시켜주는 메소드
		list.remove(1);
		...
		// 6. get(int index) : E => 리스트의 해당 index위치의 데이터를 반환해주는 메소드
...
//		System.out.println(m.getTitle());
		...
		System.out.println("======================================================");
		// for each문 : 향상된 반복문
		// for(값을 받아줄 변수 : 순차적으로 접근할 배열, 컬렉션)
		for(Music music : list) {
			System.out.println(music);
		}
		
		// 7. subList(int index1, int index2) : 해당 리스트로부터 index1에서 index2까지의 데이터 값들을 추출해서 새로운 List로 변환시켜주는 메소드
		...
		// 9. isEmpty() : 해당 리스트가 비어있는 지 묻는 메소드(비어있으면 true/ 비어있지 않다면 false 반환) // 많이 사용됨
