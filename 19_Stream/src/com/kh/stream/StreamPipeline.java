...
public class StreamPipeline {
	/*
	 * PipeLIne
	 * 데이터의 처리결과가 다음 단계의 데이터 처리에 사용되며 또 다음 데이터 처리에 사용되는
	 * 연속적인 처리구조를 의미
	 * 
	 * 파이프라인은 여러개의 스트림을 "연결"하여 사용하는데
	 * 중간처리 스트림 n개 + 최종처리 스트림 1개로 구성됨
	 * 
	 * 중간처리 스트림 역할 : 필요한 데이터 검색 및 필터링, 형변환 등(filter, map, sort...)
	 * 최정처리 스트림 역할 : 중간처리 스트림의 값을 토대로 결과값 반환(sum, min...)
	 */
	public static void main(String[] args) {
		...
		System.out.println("삼성폰 평균 가격은? "+avgPrice+"원입니다.");
