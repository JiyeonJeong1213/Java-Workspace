...
public class Run {
	 ...
	 * - 데이터를 교환하기에 앞서서 서버, 클라이언트 연결되어있어야 함(서버가 먼저 실행되면서 클라이언트의 요청을 기다림)
	 * - 신뢰성 있는 데이터 전달 가능
	 * 
	 * ServerSocket
	 ...
	 * - 메소드 
	 * 	> OutputStream out = client.getOutputStream() 메소드를 통해 데이터를 "내보낼 수 있는" 출력스트림을 생성
	 * 	> InputStream in = client.getInputStream() 메소드를 통해 데이터를 읽을 수 있는 입력 스트립을 생성
	 */
