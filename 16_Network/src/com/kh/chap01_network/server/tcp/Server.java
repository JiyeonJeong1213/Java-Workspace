...
public class Server {
	...
	public void serverStart() {
		...
		try {
			...
			while(true) {
				// 3) 접속요청이 오면 요청 수락(accept())후 해당 클라이언트에 대한 소켓객체 생성
				// 4) 클라이언트의 정보를 저장
				...
				// 5,6_2) 보조스트림을 통해 성능 개선
				// 클라이언트로부터 전달된 값을 한줄 단위로 읽어들이기 위한 입력용 스트림
				BufferedReader br = new BufferedReader(new InputStreamReader(is));
				// 클라이언트에게 값을 한줄 단위로 출력하는 스트림
				PrintWriter pw = new PrintWriter(os, true); // 데이터를 출력할 때 print(), println()메소드를 사용
				...
				while(true) {
					...
					if(message == null || "exit".equals(message)) {
						...
						pw.flush(); // 현재 스트림에 있는 데이터를 강제로 내보내는 메소드. close()메소드 호출되면 자동으로 flush()호출됨
					}
				}
				// 8) 통신종료 
				br.close();
